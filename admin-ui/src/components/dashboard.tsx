...
// 下拉一并移除。按状态过滤现在统一由 console/credential-state.ts 的 getDisposition
// 判定，并由紧贴列表的状态标签条驱动 —— 两套状态定义并存迟早会互相漂移。
...
export function Dashboard({ onLogout, embedded = false }: DashboardProps) {
  ...
  // 状态筛选：由紧贴列表的状态标签条驱动。'' = 全部
  //
  // 合并说明：PR #56 曾用一个多选下拉「按状态隐藏」（hiddenStatuses）做同一件事。
  // 两者都在按状态过滤，同一条工具栏上摆两个状态控件只会让人猜哪个生效，故只留标签条。
  // 差异是标签条一次看一类、下拉能一次隐藏多类；若确实需要"同时隐藏超额和禁用"
  // 这种组合，应把标签条改成可多选，而不是把两套控件并存。
  //
  ...
  // 各状态计数（状态标签条用）。基于全量凭据而非当前筛选结果 ——
  // 标签条是导航器，点进某一段之后其余段的数字不该跟着变。
        ...
        <div className="mb-5 flex flex-col gap-3">
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
            ...
            <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
              ...
              {/* 合并去重：PR #56 的「隐藏状态」多选下拉与紧贴列表的状态标签条
                  做的是同一件事（按状态过滤）。同一条工具栏上并存两个状态控件只会
                  让人猜哪个生效，故保留标签条、移除此下拉。
                  代价：标签条一次只看一类，下拉可一次隐藏多类；若确需组合过滤，
                  应把标签条改成可多选，而不是把两套控件都留着。 */}
            ...
            <div className="ml-auto grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
              {/* 选中后的批量操作已移到吸底批量栏，见本页底部 BulkBar
                  （PR #56 的 batchDeleting 进度态已带到那里的删除按钮上） */}
